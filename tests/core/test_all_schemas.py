...
VALID_FIXTURES["youtube_adaptation_b5_i2_package"] = {
    ...
    "input_references": {
        ...
        "evidence_or_claims_reference": {"artifact_id": "E-1", "version": "1.0.0", "checksum": "a" * 64}, "claims_ledger": {"artifact_id": "CL-1", "version": "1.0.0", "checksum": "b" * 64}, "evidence_report": {"artifact_id": "E-1", "version": "1.0.0", "checksum": "a" * 64},
    },
    ...
    "preliminary_youtube_risk_review": {
        "platform_risk": {
            ...
            "audiovisual_review_pending": True, "evidence_refs": ["E-1"]
        },
        "rights_reuse_risk": {
            "quotes": "Breves.",
            ...
            "audiovisual_review_pending": True, "evidence_refs": ["E-1"],
            "unresolved_items": [],
            "mitigations": ["Citas breves."], "evidence_refs": ["E-1"]
        }
    },
    "unsupported_elements": [],
    "unresolved_items": [],
    "producer_limits": ["No autoriza B5-I3."],
    "strategic_return": {"return_required": False, "target_domain": None, "strategic_triggers": {key: False for key in _TOPIC_TRIGGERS}, "reason": None, "evidence_refs": []},
...
VALID_FIXTURES["youtube_adaptation_review"] = {
    ...
    "unsupported_elements": [],
    "platform_risk_summary": {"severity": "LOW", "summary": "Riesgo bajo.", "mitigations": ["Contextualizar."], "uncertainties": ["Pendiente revisión audiovisual."], "evidence_refs": ["E-1"]},
    "rights_reuse_summary": {"severity": "LOW", "summary": "Uso transformativo preliminarmente aceptable.", "mitigations": ["Citas breves."], "unresolved_items": [], "evidence_refs": ["E-1"]},
...
VALID_FIXTURES["work_research_dossier"] = {
    ...
    "analysis_references": [{"analysis_id": "A-1", "material_id": "M-1", "artifact_version": "1.0.0", "artifact_checksum": "a" * 64}],
    ...
    "claim_dispositions": {"claims_ledger_id": "CL-001", "claims_ledger_version": "1.0.0", "claims_ledger_checksum": "a" * 64, "authority_status": "REPRESENTATION_ONLY_IR4_PENDING", "candidate_allowed_claim_ids": ["CLAIM-001"], "candidate_limited_claim_ids": [], "candidate_blocked_claim_ids": []},
...
VALID_FIXTURES["work_research_dossier"]["analysis_references"][0]["artifact_checksum"] = _fixture_checksum(VALID_FIXTURES["narrative_human_analysis"])
VALID_FIXTURES["work_research_dossier"]["claim_dispositions"]["claims_ledger_checksum"] = _fixture_checksum(VALID_FIXTURES["claims_ledger"])
...
for _owner, _source in (("CHANNEL_INTELLIGENCE", "policies/channel_intelligence/topic_belonging_policy.md"), ("SCRIPT_PRODUCT", "policies/script_product/main_episode_format_policy.md"), ("YOUTUBE_ADAPTATION", "config/youtube_adaptation_r3_traceability.json")):
    VALID_FIXTURES["editorial_semantic_memory"]["functional_dimension_sources"][_owner]["sources"][0]["checksum"] = hashlib.sha256((Path(__file__).resolve().parents[2] / _source).read_bytes()).hexdigest()
VALID_FIXTURES["editorial_semantic_memory"]["functional_dimension_sources"]["SCRIPT_PRODUCT"]["sources"].append({"artifact_ref": "policies/script_product/episode_discovery_and_material_curation_policy.md", "version": "1.0.0", "checksum": hashlib.sha256((Path(__file__).resolve().parents[2] / "policies/script_product/episode_discovery_and_material_curation_policy.md").read_bytes()).hexdigest()})
for _owner, _binding in VALID_FIXTURES["editorial_semantic_memory"]["functional_dimension_sources"].items():
    _paths = [Path(__file__).resolve().parents[2] / _source["artifact_ref"] for _source in _binding["sources"]]
    _dimensions = sorted(_source_dimension_universe(_owner, _paths))
...
VALID_FIXTURES["research_plan"] = {
    "contract": "research_plan", "contract_version": "2.0.0", "research_plan_id": "RP-FIXTURE",
    "episode_id": "EP-1", "brief_version": "1.0.0", "research_role": "NORMAL",
    "editorial_intent": "PREFERIDA",
    "editorial_intent_provenance": {"source": "USER_INTAKE", "reference": "INT-FIXTURE"},
    "origin": {"input_ref": "human-input:INT-FIXTURE", "source_kind": "HUMAN_EPISODE_INPUT", "question_ref": "question:EP-1"},
    "central_question": {"question": "¿Qué debemos comprender?", "decision_relevance": "Define el alcance del episodio."},
    "intended_use": {"uses": ["RESEARCH_AND_THESIS"], "required_outputs": ["claims", "limitations"]},
    "scope": {"included": ["Fenómeno y contexto"], "excluded": ["Arquitectura narrativa"]},
    "dimensions": [{"dimension_id": "D-PHENOMENON", "label": "Fenómeno", "research_question": "¿Cómo funciona?"}],
    ...
    "source_strategy": [{"strategy_id": "SS-1", "source_types": ["PRIMARY_SOURCE", "SPECIALIST_LITERATURE"], "purpose": "Contrastar el fenómeno.", "limitations": ["Acceso puede ser indirecto."]}],
    "critical_claims": [{"claim_id": "CL-1", "statement": "El fenómeno presenta esta característica.", "intended_use": "CENTRAL_CLAIM_SUPPORT", "strength": "DESCRIPTIVE", "evidence_requirement_refs": ["ER-1"], "material_if_false": True}],
    ...
    "gaps_risks": [{"gap_id": "G-1", "kind": "EVIDENCE", "description": "Puede faltar una fuente primaria.", "material_impact": "Limita el claim.", "mitigation": "Buscar fuente independiente."}],
    "potential_specialists": [{"specialist_id": "SP-1", "field": "Historia", "activation_condition": "Si el contexto histórico resulta material.", "expected_contribution": "Evaluar contexto y límites."}],
    ...
    "target_final_works_decision": {"status": "RECOMMENDED", "requested_count": 4, "decision_basis": "Fixture explícita."},
    "supplied_works": [{"work_ref": "work:M-1", "research_role": "ANCLA", "editorial_intent": "NO_DECLARADA", "origin_ref": "human-input:INT-FIXTURE"}],
    "selection_policy": {"mode": "OWNER_OR_DELEGATED", "decision_rule": "Comparar evidencia, fidelidad y complementariedad.", "reconsideration_rule": "Reabrir decisión si la investigación profunda cambia la defendibilidad.", "decision_ref": None},
    "origin_artifact_refs": [{"artifact_ref": "human-input:INT-FIXTURE", "artifact_kind": "human_episode_input", "artifact_version": "1.0.0", "checksum": None}],
...
VALID_FIXTURES["research_ready_manifest"] = {
    ...
    "research_ready_state": "RESEARCH_READY_WITH_LIMITATIONS",
    "state_bindings": {"research_stage": "READY", "research_sufficiency": "LIMITED_BUT_USABLE", "artifact_validity": "VALID", "deep_fidelity": "APROBADA_CON_LIMITES"},
    "research_artifacts": [{"artifact_id": "research:R-1", "artifact_kind": "ResearchPack", "artifact_version": "2.0.0", "checksum": "a" * 64}],
    "downstream_restrictions": [{"restriction_id": "R-1", "kind": "CLAIM_LIMITED", "statement": "Usar con límites declarados.", "affected_consumers": ["B5-I3"]}],
    "lineage": {"source_refs": ["research_plan:RP-FIXTURE"], "producer_contract": "research_pack", "producer_version": "2.0.0"},
    "created_at": "2026-08-22T00:00:00Z",
}
