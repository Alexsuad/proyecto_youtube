...
def test_p6a_topic_first_vertical_reaches_canonical_b5_i2_route(tmp_path: Path) -> None:
    ...
    active_profile = json.loads((ROOT / "config/active_editorial_profile.json").read_text(encoding="utf-8")); brief = json.loads(paths["brief"].read_text(encoding="utf-8")); brief.update({"episode_id": EP, "profile_version": active_profile["ACTIVE_PROFILE_VERSION"], "profile_checksum": active_profile["profile_checksum"]}); brief = bind_episode_brief_to_topic_decision(brief, reviewed); _write(paths["brief"], brief)
